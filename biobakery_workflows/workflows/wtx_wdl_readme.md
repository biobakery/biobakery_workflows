...
Two demo data sets are included in this workspace. A very small demo tutorial set and a set of six samples (three MTX and three MGX) from IBDMDB. The very small demo tutorial set only contains two samples and so it runs very quickly with a cost of less than $2 even if not using preemptive instances. This is a good set to get familiar with the tasks run. However, since there are only two samples with very low read count and this demo does not include metadata this is not a good set to use to get familiar with the final visualization report. Please run with the IBDMDB demo set if you would like to see a representative exploratory report with metadata. 

Very small (2 tutorial samples) demo run configuration:
* ProjectName : `"demo12"` (this can be any string you would like)
* InputExtension : `".fastq.gz"`
* InputRead1Identifier : `".R1"`
* InputRead2Identifier : `".R2"`
* InputRead1Files : `"gs://fc-7130738a-5cde-4238-b00a-e07eba6047f2/demo_files/files_list.txt"`
